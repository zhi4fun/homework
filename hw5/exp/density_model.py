...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        for i in range(states.shape[0]):
            ...
            counts = np.append(counts, self.hist[bin_name])
        return counts
...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = np.array([[(i - j) for j in self.means] for i in states])
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        #raise NotImplementedError
        self.log_likelihood = tf.squeeze(self.discriminator.log_prob(self.discrim_target), -1)
        self.likelihood = tf.squeeze(self.discriminator.prob(self.discrim_target), -1)
    ...
    def make_prior(self, z_size):
        ...
        prior_logstd = np.log(np.ones(int(z_size)), dtype=np.float32)
    ...
    def make_discriminator(self, z, output_size, scope, n_layers, hid_size):
        """
            ...
            Hint: use build_mlp
        """
        logit = build_mlp(z, output_size, scope, n_layers=n_layers, size=hid_size)
    ...
    def update(self, state1, state2, target):
        ...
        elbo = self.sess.run(self.elbo, feed_dict={
        	self.state1 : state1,
        	self.state2 : state2,
        	self.discrim_target : target
        	}) 
